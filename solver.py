...
# def solveScramble(scrambledCube, solvedCube):
...
#     solutionFound = False
#     while True:
#         print(len(scrambledStates))
#         for scramble in list(scrambledStates.keys()):
#             if scramble in solvedStates:
#                 print(scrambledStates[scramble] + solvedStates[scramble][::-1])
#                 solutionFound = True
#             elif solutionFound == False:
#                 for i in range(18):
#                     moveName1 = getMoveName(i, False)
#                     if not testAdd(scrambledStates[scramble], moveName1):
#                         continue
#                     scramble1 = applyMove(i, detupleize(scramble))
#                     for j in range(18):
#                         moveName = getMoveName(j, False)
#                         if not testAdd([moveName1], moveName):
#                             continue
#                         nextScramble = applyMove(j, detupleize(scramble1))
#                         if tupleize(nextScramble) not in scrambledStates:
#                             scrambledStates[tupleize(nextScramble)] = scrambledStates[scramble] + [moveName1] + [moveName]
...
#             for i in range(18):
#                 moveName1 = getMoveName(i, True)
#                 if not testAdd(solvedStates[scramble], moveName1):
#                     continue
#                 scramble1 = applyMove(i, detupleize(scramble))
#                 for j in range(18):
#                     moveName = getMoveName(j, True)
#                     if not testAdd([moveName1], moveName):
#                         continue
#                     nextScramble = applyMove(j, detupleize(scramble1))
#                     if tupleize(nextScramble) not in solvedStates:
#                         solvedStates[tupleize(nextScramble)] = solvedStates[scramble] + [moveName1] + [moveName]
...
# def matches(scramble, solved):
#     total = 0
#     for i in range(6):
#         for j in range(3):
#             for k in range(3):
#                 if scramble[i][j][k] == solved[i][j][k]:
#                     total += 1

#     #print(currTotal, nextTotal)
#     return total


# def isBetterScramble(currScramble, nextScramble, desiredFaceMap, idx):
#     currTotal = 0
#     nextTotal = 0

#     for i in range(3):
#         for j in range(3):
#             if currScramble[idx][i][j] == desiredFaceMap[i][j]:
#                 currTotal += 1
#             if nextScramble[idx][i][j] == desiredFaceMap[i][j]:
#                 nextTotal += 1
    
#     #print(currTotal, nextTotal)
#     return nextTotal >= currTotal


# def solveSingleSide(scrambledCube, desiredFaceMap, idx):
#     scrambledStates = dict()
#     scrambledStates[tupleize(scrambledCube)] = []

#     dfm1 = [row[:] for row in desiredFaceMap]  # Create a copy of the matrix
#     rotateSingle(dfm1)

#     dfm2 = [row[:] for row in dfm1]  # Create a copy of the rotated matrix
#     rotateSingle(dfm2)

#     dfm3 = [row[:] for row in dfm2]  # Create a copy of the doubly rotated matrix
#     rotateSingle(dfm3)
...
#         print(len(scrambledStates))
#         for scramble in list(scrambledStates.keys()):
#             currFaceMap = detupleize(scramble)[idx]
#             if currFaceMap == desiredFaceMap:
#                 print(0)
#                 print(scrambledStates[scramble])
#                 printCubeMap(scramble)
#                 return
#             elif currFaceMap == dfm1:
#                 print(1)
#                 print(scrambledStates[scramble] + [colorSideMap[colors[idx]]], "'")
#                 printCubeMap(scramble)
#                 return
#             elif currFaceMap == dfm2:
#                 print(2)
#                 print(scrambledStates[scramble] + [colorSideMap[colors[idx]]], "2")
#                 printCubeMap(scramble)
#                 return
#             elif currFaceMap == dfm3:
#                 print(3)
#                 print(scrambledStates[scramble] + [colorSideMap[colors[idx]]])
#                 printCubeMap(scramble)
#                 return
...
#                     nextScramble = applyMove(i, detupleize(scramble))
#                     moveName = getMoveName(i, False)
#                     if tupleize(nextScramble) not in scrambledStates and (isBetterScramble(scramble, nextScramble, desiredFaceMap, idx) or isBetterScramble(scramble, nextScramble, dfm1, idx) or isBetterScramble(scramble, nextScramble, dfm2, idx) or isBetterScramble(scramble, nextScramble, dfm3, idx)):
#                         if testAdd(scrambledStates[scramble], moveName):
#                             scrambledStates[tupleize(nextScramble)] = scrambledStates[scramble] + [moveName]
...
# shows that isBetterScramble is better
# singleSideSolve = [
#     ['W', 'G', 'G'],
#     ['O', 'W', 'R'],
#     ['O', 'G', 'R']
...
# def setSolve():
#     idx = colors.index(singleSideSolve[1][1])
#     solveSingleSide(solvedCube, singleSideSolve, idx)
